...
const STORAGE_KEY = "cuerocaza_cart_v1";
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  const setQty = (id: string, qty: number) => setItems((p) => qty <= 0 ? p.filter((i) => i.id !== id) : p.map((i) => i.id === id ? { ...i, quantity: qty } : i));
  const clear = () => setItems([]);

  const count = items.reduce((s, i) => s + i.quantity, 0);
  const subtotalCents = items.reduce((s, i) => s + (i.price_cents ?? 0) * i.quantity, 0);
